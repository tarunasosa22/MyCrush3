...
import { staticPublicKey } from '../constant';
...
interface FirebaseRemoteAdsConfigTypes {
  adsShowType: string;
  admobAppId: string;
...
interface AdsStoreTypes {
  isAdsVisible: boolean;
  isAdsVisibleRemote: boolean;
  remoteData: FirebaseRemoteAdsConfigTypes;
  adsCount: number;
  createAvatarAdsCount: number;
  createAvatarNativeAdsCount: number;
  homeNativeAdsCount: number;
  ...
  setRemoteData: (config: FirebaseRemoteAdsConfigTypes) => void;
...
export const useAdsStore = create<AdsStoreTypes>()(
  persist(
    (set, get) => ({
      remoteData: {
        adsShowType: '0',
